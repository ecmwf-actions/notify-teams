 ...
 */
const getAllJobs = (job, jobContext, needsContext) => {
    const jobs = [];
...
const helper_functions_1 = __nccwpck_require__(8123);
/**
 ...
 */
const getPreviousConclusion = async (repository, branch, workflowId, githubToken) => {
    ...
    try {
        ...
        if ((0, helper_functions_1.isError)(response.status != 200, `Wrong response code while fetching workflow runs for ${repo}: ${response.status}`))
            return previousConclusion;
        workflowRuns = response.data.workflow_runs || [];
    }
    catch (error) {
        if (error instanceof Error)
            (0, helper_functions_1.isError)(true, `Error fetching workflow runs for ${repo}: ${error.message}`);
        return previousConclusion;
    }
    ...
    core.info(`==> lastRunId: ${(lastRun === null || lastRun === void 0 ? void 0 : lastRun.id) || 'unknown'}`);
    previousConclusion = (lastRun === null || lastRun === void 0 ? void 0 : lastRun.conclusion) || 'unknown';
    core.info(`==> previousConclusion: ${previousConclusion}`);
    core.endGroup();
    return previousConclusion;
};
exports["default"] = getPreviousConclusion;
 ...
 */
const isError = (condition, errorMessage) => {
    if (condition) {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const core = __importStar(__nccwpck_require__(2186));
const get_all_jobs_1 = __importDefault(__nccwpck_require__(4634));
const get_previous_conclusion_1 = __importDefault(__nccwpck_require__(7238));
const send_teams_message_1 = __importDefault(__nccwpck_require__(5562));
/**
...
const main = async () => {
    try {
        ...
        const jobs = (0, get_all_jobs_1.default)(job, jobContext, needsContext);
        const previousConclusion = await (0, get_previous_conclusion_1.default)(repository, branch, workflowId, githubToken);
        const result = await (0, send_teams_message_1.default)(jobs, previousConclusion, notifyOn, repository, branch, sha, workflow, runId, incomingWebhook);
        const outputs = {
 ...
 */
const sendTeamsMessage = async (jobs, previousConclusion, notifyOn, repository, branch, sha, workflow, runId, incomingWebhook) => {
    ...
    if ((notifyOn.includes('failure')
        ...
        try {
            const response = await new http_client_1.HttpClient().postJson(incomingWebhook, messageCard);
            if ((0, helper_functions_1.isError)(response.statusCode != 200, `Wrong response code while sending message to MS Teams: ${response.statusCode}`))
                return Promise.reject(result);
        }
        catch (error) {
            if (error instanceof Error)
                (0, helper_functions_1.isError)(true, `Error sending message to MS Teams: ${error.message}`);
            return Promise.reject(result);
        }
...
exports["default"] = sendTeamsMessage;
